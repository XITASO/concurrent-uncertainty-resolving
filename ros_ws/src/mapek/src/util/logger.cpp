 ...
 *
 */
BTLogger::BTLogger() : Node("BT_Logger")
{
    this->set_parameter(rclcpp::Parameter("use_sim_time", true));
    // Define the QoS settings
    rclcpp::QoS qos_settings = rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(rmw_qos_profile_sensor_data));
    ...
    clock_sub = this->create_subscription<rosgraph_msgs::msg::Clock>("/clock", qos_settings, std::bind(&BTLogger::clock_callback, this, _1));
}
...
{
    return static_cast<long long>(time.sec) * 1000000000LL + static_cast<long long>(time.nanosec);
}
