...
#include <system_modes_msgs/msg/mode.hpp>
...
system_interfaces::msg::CommChangeInput toCommChangeAdaptation(GenericAdaptation);
system_interfaces::msg::ParametrizationInput toParametrizationAdaptation(GenericAdaptation);
std::vector<lifecycle_msgs::msg::Transition> toLifecyleAdaptation(GenericAdaptation);
system_modes_msgs::msg::Mode toSystemMode(GenericAdaptation);

GenericAdaptation toGenericAdaptation(system_interfaces::msg::CommChangeInput);
GenericAdaptation toGenericAdaptation(system_interfaces::msg::ParametrizationInput);
GenericAdaptation toGenericAdaptation(lifecycle_msgs::msg::Transition);
GenericAdaptation toGenericAdaptation(system_modes_msgs::msg::Mode);

int getAdaptationExecutionEstimate(uint8_t adaptation_type);


bool sanityCheck(GenericAdaptation);
...
size_t countAdaptations(const std::vector<GenericAdaptation>& adaptations, const std::vector<uint8_t>& desired_actions);

}

// hash specialization, to use GenericAdaptation in unordered_map etc
template <>
struct std::hash<GenericAdaptation>
{
  std::size_t operator()(const GenericAdaptation& adaptation) const
  {
    using std::size_t;
    using std::hash;
    using std::string;
