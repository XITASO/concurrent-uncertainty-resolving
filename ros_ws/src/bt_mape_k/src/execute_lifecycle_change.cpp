...
BTExecuteLifeCycle::BTExecuteLifeCycle(const std::string &name, const BT::NodeConfiguration &config)
: BT::StatefulActionNode(name, config)
{
    node_ = rclcpp::Node::make_shared("execute_lifecycle_action_node");
    component_name = getInput<std::string>("component_name").value();
    service_handler_ = std::make_unique<LifecycleChangeServiceHandler>(node_, component_name);
}

BT::NodeStatus BTExecuteLifeCycle::onStart()
{
...
BT::NodeStatus BTExecuteLifeCycle::onRunning()
{
    MAPEK_Graph graph = getInput<MAPEK_Graph>("system_state_graph").value();
    switch (service_handler_->tick(graph))
    {
...
void BTExecuteLifeCycle::onHalted() { }

BT::PortsList BTExecuteLifeCycle::providedPorts()
{
    return {BT::InputPort<std::string>("component_name"),
            ...
            BidirectionalPort<AdaptationMap>("finished_adaptations"),
