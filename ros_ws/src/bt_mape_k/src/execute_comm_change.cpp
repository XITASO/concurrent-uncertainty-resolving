...
BTExecuteCommChange::BTExecuteCommChange(const std::string &name, const BT::NodeConfiguration &config)
    : BT::StatefulActionNode(name, config)
{
    node_ = rclcpp::Node::make_shared("execute_comm_change_action_node");
    std::string component_name = getInput<std::string>("component_name").value();
    service_handler_ = std::make_unique<CommChangeServiceHandler>(node_, component_name);
}

BT::NodeStatus BTExecuteCommChange::onStart()
{
...
BT::NodeStatus BTExecuteCommChange::onRunning()
{
    MAPEK_Graph graph = getInput<MAPEK_Graph>("system_state_graph").value();
    switch (service_handler_->tick(graph))
    {
...
void BTExecuteCommChange::onHalted() {}

BT::PortsList BTExecuteCommChange::providedPorts()
{
