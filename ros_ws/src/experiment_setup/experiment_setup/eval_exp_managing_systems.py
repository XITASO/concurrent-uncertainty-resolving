...
def main() -> None:
    log_file_folder = "/home/dockuser/ros_ws/log_dump/robustness_analysis/"
    log_file_folders = [
        os.path.join(log_file_folder, "acsos1"),
        os.path.join(log_file_folder, "acsos2"),
        os.path.join(log_file_folder, "acsos3"),
    ]
    scenarios = []
    ...
    for r in ["settings"]:
        for m in [
            ...
            scenarios.append(r + m)
    
    overall_result = {}
    per_run_data: Dict[str, list] = {}
    #total_files = sum(len(os.listdir(os.path.join(log_file_folder, scenario))) for scenario in scenarios)
    total_files = 0
    valid_log_files = 0
    for scenario_folder in scenarios:
        ...
        per_run_data[scenario_folder] = []

        for log_file_folder in log_file_folders:
            for single_run in sorted(
                ...
                per_run_data[scenario_folder].append({
                    "down_time": int(down_time) if down_time is not None else "",
                    "strategy_success": relative_strategy_success if relative_strategy_success is not None else "",
                    "unn_redeploys": unn_redeploys,
                    "reaction_times": [int(single_reaction_time) for single_reaction_time in reaction_time] if reaction_time else [],
                })
    ...
    print(f"Valid log files: {valid_log_files} / {total_files}")

    print_latex(result_dict=overall_result)
    write_exploded_csv("/home/dockuser/ros_ws/output_exploded_robustness_analysis.csv", per_run_data)
...
def write_exploded_csv(
    ...
    """Write per-reaction-time experiment data to a CSV file.

    The format matches output_exploded.csv:
        setting,scenario,down_time,reaction_time,strategy_success,unn_redeploys

    Each row represents one individual reaction time. The ``scenario`` column
    contains the zero-based run index within the given setting. The first row
    for a run keeps the run-level metrics. Additional rows for the same run
    leave the other metric columns empty.
    ...
    per_run_data:
        Mapping from setting name (e.g. ``"settings_deps_crit"``) to a list
        of dicts, one dict per run, with keys ``down_time``,
        ``strategy_success``, ``unn_redeploys``, and ``reaction_times``.
    """
    with open(output_path, "w", newline="") as f:
        ...
        for setting, runs in per_run_data.items():
                ...
                if not reaction_times:
                    writer.writerow(
                        [
                            ...
                            run["down_time"],
                            "",
                            run["strategy_success"],
                            run["unn_redeploys"],
                        ]
                    )
                    continue

                for reaction_time_idx, reaction_time in enumerate(reaction_times):
                    writer.writerow(
                        [
                            setting,
                            idx,
                            run["down_time"] if reaction_time_idx == 0 else "",
                            reaction_time,
                            run["strategy_success"] if reaction_time_idx == 0 else "",
                            run["unn_redeploys"] if reaction_time_idx == 0 else "",
                        ]
                    )
