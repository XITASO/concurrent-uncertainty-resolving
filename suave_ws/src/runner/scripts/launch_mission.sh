...
if [ $# -lt 1 ]; then
	...
	echo example:
	echo "  "$0 "[metacontrol | random | none | bt | bt_mapek] [time | distance | extended]"
	exit 1
fi
...
if [ "$1" == "metacontrol" ] || [ "$1" == "random" ] || [[ "$1" == "none" ]] || [[ "$1" == "bt" ]] || [[ "$1" == "bt_mapek" ]];
then
    MANAGER=$1
else
...
if [ "$MANAGER" == "metacontrol" ] || [ "$MANAGER" == "random" ] || [[ "$MANAGER" == "none" ]] || [[ "$MANAGER" == "bt_mapek" ]];
then
