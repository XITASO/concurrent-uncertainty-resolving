...
if [ $# -lt 1 ]; then
	...
	echo example:
	echo "  "$0 "[true | false] [metacontrol | random | none | bt | rosa | bt_mape_k] [time | distance | extended] runs(integer)"
	exit 1
fi
...
if [ "$2" == "metacontrol" ] || [ "$2" == "random" ] || [ "$2" == "none" ] || [ "$2" == "bt" ] || [ "$2" == "rosa" ] || [ "$2" == "bt_mape_k" ];
then
    MANAGER=$2
else
    echo "adaptation_manager invalid or missing in the beginning"
    exit 1
fi
...
sleep 1
for i in $(ps -aux | grep typedb | grep -v grep | awk '{print $2}')
do
    echo "kill -2 $i"
    kill -2 $i;
done
...
run_missions(){
  ...
  do
      ...
      setsid ./scripts/start_ardusub.sh $GUI &
      sleep 10 #let it boot up
      setsid ./scripts/launch_sim.sh $GUI &
      sleep 30 #let it boot up

      rm -f /tmp/mission.done
      setsid ./scripts/launch_mission.sh $MANAGER $MTYPE $FILENAME &
      sleep 30 #let it boot up
